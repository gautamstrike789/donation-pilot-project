...
def forms_value(form_rows):
    return f"{len(form_rows)}f/{round(form_rows['_supports'].sum(), SUPPORTS_DECIMALS):g}s"


def render_weekly_summary(code):
    """WE date + one row of the owner's totals (all BAs combined) for each
    day of the chosen week and the week as a whole, as
    "<forms>f/<supports>s", defaulting to the current week; then the owner's
    totals for a chosen month."""
    try:
    ...
    st.markdown(summary_table_html(cells, "Forms / Supports", forms_value(week_rows)), unsafe_allow_html=True)
    ...
    ym = mc2.selectbox("Month", month_options, index=month_options.index(current_month),
                       format_func=lambda m: datetime(m[0], m[1], 1).strftime("%B %Y"), key=f"month_sel_{code}")
    ...
    st.markdown(
        ...
        f'<span>total forms / supports submitted in {month_name}</span></div>',
...
def start_submit_job(batch_id, kind, code, rows, new_bas=()):
    """kind: "donations" or "noprod". new_bas: staged (OWNCODE, Client, BACode, BAName, NewJoineeDate)."""
    cfg = load_config()
    creds = get_credentials()
    return submit_worker.start_job(
        ...
        bas=submit_worker.SheetRef(creds, cfg[ADMIN_SHEET_KEY], "BAs"),
        donations=submit_worker.SheetRef(creds, cfg[DONATIONS_SHEET_KEY]),
        submitlog=submit_worker.SheetRef(creds, cfg[ADMIN_SHEET_KEY], "SubmitLog"),
        batch_id=batch_id,
        code=code,
        donation_rows=[[r[h] for h in HEADERS] for r in rows],
        # BAs worksheet column order: OWNCODE, BACode, BAName, Client, New Joinee Date
        new_ba_rows=[[o, cd, nm, ocl, jd] for o, ocl, cd, nm, jd in new_bas],
    )
...
def apply_finished_submit_job():
    ...
    if bas_written:
        load_admin.clear()
        for owner_code, owner_client, cd, effective_ba, _jd in meta["new_bas"]:
            st.session_state.new_bas.setdefault((owner_code, owner_client), []).append((effective_ba, cd))
    ...
    if result is not None:
        ...
        if meta["kind"] == "noprod":
            ...
            st.session_state.flash_success = (
                f"✅ Submitted {len(rows)} entry(s) across {final_ba_count} BA(s)."
            )
    ...
    if getattr(err, "stage", None) == "admin":
        ...
        msg = f"Couldn't save to Donations One-Off sheet: {cause}"
    st.session_state.submit_error = {
        "msg": msg,
        "perm": getattr(err, "stage", None) != "admin" and ("403" in str(cause) or "PERMISSION_DENIED" in str(cause)),
        "bas_added": ", ".join(f"{nm} ({cd})" for _o, _ocl, cd, nm, _jd in meta["new_bas"]) if bas_written else "",
    }
    ...
    if not signin:
        ...
        if not no_production:
            ...
            if st.session_state.pending_preview:
                ...
                if submit_clicked:
                    ...
                    if edit_errors:
                        ...
                        # The writes (SubmitLog check → new BAs → entries → log) run on a
            ...
            nf_submit_clicked = st.button("✅ Submit", type="primary", use_container_width=True,
                                          key=f"nf_submit_{n}", disabled=submit_in_flight)
            if submit_in_flight:
                submit_progress()
            show_submit_error()
